...
2. **preview.html 单文件预览要求**（最重要！这是用户看到的预览效果）：
   ...
   - **【重要】必须是桌面端网页设计，禁止手机APP风格**：
     * 适配电脑浏览器，最小内容宽度 1024px，充分利用宽屏空间
     * 禁止使用移动端APP专属设计：底部Tab导航栏、超大圆角按钮、居中窄屏容器（max-w-sm/max-w-md）、汉堡菜单作为主导航
     * 使用桌面端常见布局：顶部导航栏、左侧侧边栏、多列网格布局、宽屏内容区
     * 按钮、文字大小符合桌面端使用习惯，不要做手机端的大点击区域
     * 不需要专门适配手机端，只需要保证在电脑浏览器上显示美观
